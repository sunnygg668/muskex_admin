...
use app\common\library\CustomSms;
...
class Sms
{
    ...
    public static function send($templateCode, $mobile, $tplVar = []): bool
    {
        ...
        if(in_array('smsbao', $config['default']['gateways'])) {//短信宝固定模版
            $templateData['content'] = '【名耀】您的验证码${code}，该验证码5分钟内有效';
        }
        // 解析模板
        ...
        try {
            if(in_array('smsbao', $config['default']['gateways'])) {//自定义短信运营商
                $config['default']['gateways'] = ['smsbao'];
                $easySms = new CustomSms($config);
            }elseif(in_array('maixuntong', $config['default']['gateways'])) {//自定义短信运营商
                $config['default']['gateways'] = ['maixuntong'];
                $easySms = new CustomSms($config);
            }else{
            ...
            [
                ...
                'children'  => [
                    ...
                    [
                        ...
                        'icon'      => 'fa fa-asterisk',
                        'menu_type' => 'tab',
                        'component' => '/src/views/backend/sms/variable/index.vue.bak',
                        'keepalive' => '1',
                        'children'  => [
                    ...
                    [
                        ...
                        'menu_type' => 'tab',
                        'component' => '/src/views/backend/sms/template/index.vue.bak',
                        'keepalive' => '1',
    ...
    {
        Menu::enable('sms');
        Config::addQuickEntrance('短信配置', 'sms/config');
