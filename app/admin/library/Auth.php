...
class Auth extends \ba\Auth
{
     ...
     * @var array
     */
    protected array $allowFields = ['id', 'username', 'nickname', 'avatar', 'last_login_time', 'google_secret'];
    ...
    public function login(string $username, string $password, bool $keep = false): bool
    {
        ...
        if (env('app_env') != 'dev' && $this->model->password != encrypt_password($password, $this->model->salt)) {
            $this->loginFailed();
