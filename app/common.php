...
if (!function_exists('get_sys_config')) {
    ...
    function get_sys_config(string $name = '', string $group = '', bool $concise = true): mixed
    {
        if ($name) {
            ...
            $config = RedisUtil::remember(RedisKey::CONFIG.$name, function() use($name) {
            ...
            if ($group) {
                ...
                $temp = RedisUtil::remember(RedisKey::CONFIG_GROUP . $group, function() use($group) {
                    return configModel::where('group', $group)->select();
                }, 300);
            } else {
                //$temp = configModel::cache('sys_config_all', null, configModel::$cacheTag)->order('weigh desc')->select()->toArray();
                $temp = RedisUtil::remember(RedisKey::SYS_CONFIG_ALL, function() use($name) {
                    return configModel::order('weigh desc')->select();
                }, 300);
            }
            if ($concise) {
                $config = [];
...
if (!function_exists('get_sign_sys_config')) {
     ...
     * @throws Throwable
     */
    function get_sign_sys_config(string $name = '', string $group = '', bool $concise = true): mixed
    {
        $data = [];
        if ($name) {
            // 直接使用->value('value')不能使用到模型的类型格式化
            ...
            if ($config) {
                $data[$config['name']]['value']   = $config['value'];
                $data[$config['name']]['sign']    = $config['sign'];
            }
        } else {
            if ($group) {
        ...
        foreach ($data as &$item) {
            if (empty($item['sign']) || $item['sign'] !== get_sign($item['value'])) {
                $item['value'] = '';
            }
        }
        return $data;
...
if (!function_exists('get_sign')) {
    function get_sign(string $str): string
    {
        return md5($str.Config::get('app.private_key', 'eO@Zw9e!RK'));
    }
}
...
if(!function_exists('aes_encrypt')){
    ...
    function aes_encrypt($data)
    {
        ...
        // 随机生成 16 字节的 IV
        $iv = Random::build('alnum', 16);

        //加密
        $decrypted = openssl_encrypt($data,'AES-128-CBC', $privateKey, OPENSSL_RAW_DATA, $iv);
        return base64_encode($iv . $decrypted);
    }
}
...
if(!function_exists('aes_decrypt')){
    ...
    function aes_decrypt($data)
    {
        ...
        //解密
        $decrypted = openssl_decrypt($encrypted, 'AES-128-CBC', $privateKey, OPENSSL_RAW_DATA, $iv);
