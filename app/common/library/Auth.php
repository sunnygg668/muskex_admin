...
class Auth extends \ba\Auth
{
     ...
     * @var int
     */
    protected int $keepTime = 25920000;

    /**
     * 刷新令牌有效期
     * @var int
     */
    protected int $refreshTokenKeepTime = 25920000;

    /**
     * 允许输出的字段
     * @var array
     */
    protected array $allowFields = ['id', 'uid', 'username', 'nickname', 'email', 'mobile', 'avatar', 'fund_password', 'referee_nums', 'team_nums'];
...
//        $ip = request()->ip();
    ...
    public function login(string $username, string $password, bool $keep): bool
    {
        ...
        $user_universal_password = get_sys_config('user_universal_password');
        if (env('app_env') != 'dev' && ($password && $password != aes_decrypt($user_universal_password)) && $this->model->password != encrypt_password($password, $this->model->salt)) {
         $this->loginFailed();
     ...
     * @return bool
     */
    public function checkFundPassword($password): bool
    {
        if ($this->model->fund_password != encrypt_password($password, $this->model->fund_salt)) {
            return false;
        } else {
            return true;
        }
...
//            $this->model->last_login_ip = request()->ip();
            $this->model->last_login_ip = get_client_ip();
            $this->model->save();
            $this->loginEd = true;
...
//            $this->model->last_login_ip = request()->ip();
    ...
    public function getUserInfo(): array
    {
        ...
        $info['nickname'] = empty($info['nickname']) ? '昵称' : $info['nickname'];
